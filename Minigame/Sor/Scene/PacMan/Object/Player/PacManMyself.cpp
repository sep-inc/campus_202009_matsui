...
PacManMyself::PacManMyself(PacManStage* stage_, PacManItem* item_) :
	PacManPlayer(stage_, item_, PACMAN_MYSELF, "●"),
...
{
	m_pos = Vec2(MYSELF_INIT_POS_X, MYSELF_INIT_POS_Y);   //!座標
...
void PacManMyself::SelectDirection()
{
	...
	{
		m_direction = Vec2(0, 0);  //!方向ベクトルを0にする
...
void PacManMyself::Collision()
{
	...
	//!移動先でアイテムにぶつかっている場合
	else if (m_stage->SearchObject(pos) == PACMAN_ITEM)
	{
		m_item->SetLostItem(pos);  //!どこのアイテムに当たったかを知らせる
		m_item_counter++;          //!取得アイテム数加算

		//!全てのアイテムを集めた場合
		if (m_item_counter == PACMAN_ITEM_NUM)
		{
	...
	//!移動先で敵にぶつかっている場合
	else if (m_stage->SearchObject(pos) == PACMAN_ENEMY)
	{
