...
PacManEnemy::PacManEnemy(PacManStage* stage_, PacManItem* item_) :
	PacManPlayer(stage_, item_, PACMAN_ENEMY, "＠"),
	m_ai(nullptr),      //!AI
...
void PacManEnemy::RandCreate()
{
	...
	while (true)
	{
		m_pos.x = rand() % PACMAN_GAME_WIDTH;   //!座標指定 
		m_pos.y = rand() % PACMAN_GAME_HEIGHT;	 //!座標指定

		//!指定座標箇所が壁であるかどうか
		if (m_stage->SearchObject(m_pos) != PACMAN_WALL)
		{
			//!生成間隔範囲内に他のアイテム、プレイヤー、敵がいるかどうか
			if (SearchRange(m_pos) == false)
			{
				//!配置できるならステージに情報を送る
				m_stage->SetUpStageBuffer(m_pos, m_pos, PACMAN_ENEMY);
				break;
			}
		}
	}
...
//!自分の周り調査関数
bool PacManEnemy::SearchRange(Vec2 pos_)
{
	Vec2 range;  //!調査座標
