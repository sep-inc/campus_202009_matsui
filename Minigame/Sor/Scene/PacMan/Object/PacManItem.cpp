...
{
	//!存在フラグ初期化
	for (int i = 0; i < PACMAN_ITEM_NUM; i++)
	{
...
void PacManItem::Update()
{
	for (int i = 0; i < PACMAN_ITEM_NUM; i++)
	{
		...
		{
			m_stage->SetUpStageBuffer(m_item_info[i].m_pos, m_item_info[i].m_pos, PACMAN_ITEM);
		}
	}
}
...
void PacManItem::RandCreate()
{
	for (int i = 0; i < PACMAN_ITEM_NUM; i++)
	{
		...
		while (true)
		{
			m_item_info[i].m_pos.x = rand() % PACMAN_GAME_WIDTH;
			m_item_info[i].m_pos.y = rand() % PACMAN_GAME_HEIGHT;

			//!指定座標が壁であるかどうか
			if (m_stage->SearchObject(m_item_info[i].m_pos) != PACMAN_WALL)
			{
				//!生成間隔範囲内に何もない場合
...
bool PacManItem::SearchRange(Vec2 pos_)
{
	...
	for (int y = 0; y < ITEM_RANGE_Y; y++)
	{
		for (int x = 0; x < ITEM_RANGE_X; x++)
		{
			...
			//!座標がステージの範囲内である場合
			if (range.x > 0 && range.x < PACMAN_GAME_WIDTH &&
				range.y > 0 && range.y < PACMAN_GAME_HEIGHT)
			{
				//!範囲内に何かある場合
				if (m_stage->SearchObject(range) != PACMAN_WALL &&
					m_stage->SearchObject(range) != PACMAN_NONE)
				{
					return true;
				}
			}
