...
bool ChaseAI::SearchPlayer(Vec pos_)
{
	...
	for (int y = pos_.y; y > 0; y--)
	{
		...
		if (m_stage->SearchObject(enemy_pos) == MYSELF)
		{
			m_direction = enemy_pos;
			return true;
		}
		//!壁がある場合調べるのをやめる
		else if (m_stage->SearchObject(enemy_pos) == WALL)
		{
			break;
		}
	}
	...
	for (int y = pos_.y; y < GAME_HEIGHT; y++)
	{
		enemy_pos.y = y;
	...
	for (int x = pos_.x; x < GAME_WIDTH; x++)
	{
		enemy_pos.x = x;
...
//!ランダム方向選択関数
Vec ChaseAI::RandDirection(Vec pos_)
{
...
//!当たり判定関数(壁)
bool ChaseAI::CollisionWall(Vec pos_, Vec direction_)
{
	Vec pos = pos_;   //!座標保存用
	pos.x += direction_.x;
	pos.y += direction_.y;
...
//!後退消去関数
void ChaseAI::SetDirection(Vec vec_)
{
	//!移動可能配列を一度初期化
	for (int i = 0; i < DIRECTION_MAX; i++)
	{
